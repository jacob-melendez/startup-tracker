...
| The outreach pitch: the `mailto:` subject and body, the LinkedIn connection note, who to ask for first, and the two guard lists that mark a shared inbox and an EDGAR filing entity | `config/outreach.yaml` | `web/`, which renders the drafts but writes none of them — the pitch is the variable worth rewriting after every ten sends, and rewriting it must never be a code change (SPEC §12 Phase 8) |
