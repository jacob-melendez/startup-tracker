...
| The outreach pitch: the `mailto:` subject and body, the LinkedIn connection note, who to ask for first, and the two guard lists that rank a shared inbox below a named human and keep an EDGAR filing entity from being offered as one | `config/outreach.yaml` | `web/`, which renders the drafts but writes none of them — the pitch is the variable worth rewriting after every ten sends, and rewriting it must never be a code change (SPEC §12 Phase 8) |
